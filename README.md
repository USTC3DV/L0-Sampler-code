...
We introduce the L0-Sampler, an upgraded version of NeRF's Hierarchical Volume Sampling (HVS) strategy. Through the use of piecewise interpolation, we fit a quasi-L0 function, denoted as w(t), which closely resembles an indicator function capturing the L0 distance between points and surfaces. By normalizing this function, we create a probability density function (PDF) that effectively guides inverse transform sampling. With minimal impact on training time and straightforward code modifications, our method consistently enhances rendering quality, making it a valuable addition to the NeRF framework.
